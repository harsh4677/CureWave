...
const BedPieChart = ({ beds }: { beds: Bed[] }) => {
  // Calculate totals for each category across all bed types
  const totalOccupied = beds.reduce((sum, bed) => sum + bed.occupied, 0);
  const totalReserved = beds.reduce((sum, bed) => sum + bed.reserved, 0);
  const totalAvailable = beds.reduce((sum, bed) => sum + (bed.total - bed.occupied - bed.reserved), 0);
        ...
        <div className="mb-8">
          ...
          <div className="bg-gray-50 p-4 rounded-lg">
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
              <div>
                <h3 className="text-md font-medium mb-2 text-center">Bed Status Breakdown</h3>
                <BedPieChart beds={departmentInfo.beds} />
              </div>
              <div>
                <h3 className="text-md font-medium mb-2 text-center">Bed Type Distribution</h3>
                <BedChart beds={departmentInfo.beds} />
              </div>
            </div>
          </div>
